...
export default function App() {
  ...
  const fetchData = () => {
    ...
    // 👇 Sekarang kita nembak ke server Vercel kita sendiri, BUKAN Google! 👇
    axios
      .get("/api/laporan", {
        headers: { "x-secret-pin": "123456" }, // Tiket masuknya, Bos!
      })
      .then((response) => {
        // Data sudah dikelompokkan dengan rapi dari server
        const dataTxCSV = response.data.transaksi;
        const dataClCSV = response.data.closing;
        ...
        const parseCSV = (data) =>
          data
            ...
            .filter((row) => row.trim() !== "")
            .map((row) => row.split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/))
            .filter((cols) => cols[0] && cols[0].replace(/"/g, "").trim() !== "");
        ...
        const tx = parseCSV(dataTxCSV).map((c) => ({
          nama: c[0]?.replace(/"/g, "").trim(),
          bruto: cleanNumber(c[1]),
          uang: cleanNumber(c[2]),
          periode: c[3]?.replace(/"/g, "").trim(),
          ket: c[4] ? c[4].replace(/"/g, "").trim() : "-",
        }));

        const cl = parseCSV(dataClCSV).map((c) => ({
          periode: c[0]?.replace(/"/g, "").trim(),
          fuso: parseFloat(cleanNumber(c[1])) || 0,
          kontainer: parseFloat(cleanNumber(c[2])) || 0,
          transport: cleanNumber(c[3]),
          buruh: cleanNumber(c[4]),
          total: cleanNumber(c[5]),
        }));

        setDataTransaksi(tx);
        setDataClosing(cl);

        const pList = [...new Set(tx.map((d) => d.periode))].filter((p) => p && p !== "ALL" && p !== "UNIT" && !p.includes("00"));
        setPeriodeList(["All", ...pList]);

        if (pList.length > 0) {
          const periodeClosing = pList.filter((p) => !p.toUpperCase().includes("BELUM"));
          if (periodeClosing.length > 0) {
            setSelectedPeriode(periodeClosing[periodeClosing.length - 1]);
          } else {
